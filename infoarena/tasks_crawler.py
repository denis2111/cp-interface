...
from config import *
from crawler.tasks_crawler_interface import TasksCrawlerInterface


class InfoarenaTasksCrawler(TasksCrawlerInterface):
    @staticmethod
    def get_task(link: str, force_update=False) -> Task:
        ...
        :return:
        """
        task = Task(task_id=INFOARENA_PREFIX + '-' + link.rsplit('/', 1)[-1], link_name=link.rsplit('/', 1)[-1], task_link=link)
        ...
        InfoarenaTasksCrawler._extract_info_from_table_task_page(task, info_table)
        ...
        InfoarenaTasksCrawler._extract_examples_from_table(task, examples_table)
    ...
    @staticmethod
    def get_tasks_from_page(link: str, force_update=False) -> List[Task]:
        """
        ...
        for task in tasks_text:
            ...
            task = InfoarenaTasksCrawler.get_task(task_url, force_update)
            tasks.append(task)
    ...
    def get_all_tasks(force_update=False):
        ...
        tasks = InfoarenaTasksCrawler.get_tasks_from_page(links.tasks_page, force_update)
        visited_links = set()
        for tasks_page in tasks_pages:
            ...
            tasks.extend(InfoarenaTasksCrawler.get_tasks_from_page(tasks_page_link, force_update))
